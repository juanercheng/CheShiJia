    ...
    $.page = function () {
        ...
        $page.methods = {
            		...
            		if($(".checkT .checked").length<=0){
                        ...
                        layer.confirm('你确定要删除吗?', function(index){
                           //获取选择的id
            	        	...
            	        	$.ajax({
                                 type: "POST", //提交方式
                                 url: root_path + "market/delSearchCar", //绝对路径
                                 data: {
                        ...
                        if (Response.rspCode == "0000") {
                            ...
                            var thisDate = function (curr) {
                                ...
                                for (var i = (curr * nums - nums); i <= last; i++) {
                                    ...
                                    str += '<li data-id='+ data[i].id +' >'
                                        ...
                                        +   '<p>外观/内饰颜色：' + data[i].color1 + '/' + data[i].color2 + '</p>'
                                        +'</div>'
                                        +'<span class="Hasbeen" data-id='+ data[i].id +'>编辑</span>'
                            ...
                            laypage({
                                ...
                                jump: function (obj, first) {
                                	if(market==""){
                                    	$('.ALLcheck').html(thisDate(obj.curr));
                                    }else if(market=='0'){
                                    	$('.downfinished').html(thisDate(obj.curr));
                                    }else if(market=='1'){
                                    	$('.upfinished').html(thisDate(obj.curr));
                                    }
                                	...
                                	$(".checkT>li .Hasbeen").click(function(){
                                            ...
                                            window.location.href="main_find_car_want.html?id="+id
                                    });
            ...
            pageEvent: function () {
                ...
                //点击编辑按钮跳转至我的寻车编辑页面；
                $('.Hasbeen').click(function () {
                    var uid = $(this).attr('data-id');
                    // console.log(uid);
                    window.location.href = "main_find_car_want.html?id=" + uid;
                });
